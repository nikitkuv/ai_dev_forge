...
Обновление вышедшего релиза — одна детерминированная локальная команда. Положите новый bundle в `.ai-next/` (копия или однострочник sparse-clone из [MIGRATION.md](MIGRATION.md)) и запустите из корня проекта:
...
Preview не пишет ничего: он возвращает полный план (замена framework-файлов, доказанные удалениями устаревшие пути, рендер роутеров и адаптеров, версия и явные решения конфигурации, офлайн-классификация интеграций, защищённые пути) и `preview_token`. Apply выполняет одну транзакцию с backup-журналом и полным откатом, валидацией, записью `.ai/framework.lock` последним и удалением `.ai-next/` после успеха. Версионные правила миграции объявлены машинно в `.ai/framework/migrations.yaml`; человеческая документация — в MIGRATION.md. Скилл `forge-migrate-framework` прогоняет весь цикл и собирает редкие решения пользователя; проектам без Python остаётся ручной путь.
