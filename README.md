...
`.ai/project.yaml` хранит один явный `role_execution.mode` для `epic-planner` и `reviewer`: `claude_with_codex` запускает Forge в Claude Code и делегирует обе роли напрямую установленному Codex CLI через stable `codex exec` в fresh ephemeral read-only `gpt-6-sol/high`; `codex_with_claude` запускает Forge в Codex и вызывает установленный Claude Code CLI 2.1.203+ через fresh `claude -p`, JSON output, plan mode и ограниченные read-only tools; `native_subagents` использует внутренних агентов активной платформы — Codex, Claude Code или OpenCode — без внешнего preflight. Для OpenCode-led bootstrap при отсутствии утверждённого route предлагается существующий `native_subagents`, но он всё равно записывается только после явного подтверждения; новых режимов нет.
...
Отдельно клонировать Forge рядом с проектом не нужно. Из корня обновляемого проекта один раз выполните подходящую команду staging из раздела [«Подготовить `.ai-next/`»](MIGRATION.md#подготовить-ai-next): она делает временный sparse clone и оставляет в проекте только `.ai-next/`.

Затем откройте обновляемый проект в Codex/Claude Code/OpenCode и скажите:
...
Агент запускает preview, переводит findings в понятный отчёт, сохраняет token и подставляет его в apply после вашего подтверждения. Вручную вы выбираете только реальные неоднозначности и утверждаете итоговый scope. Если актуальный клон Forge уже есть локально, его можно использовать как альтернативный источник без скачивания; оба варианта описаны в [MIGRATION.md](MIGRATION.md).
