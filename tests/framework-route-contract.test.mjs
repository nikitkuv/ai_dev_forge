...
test("manifest declares the three role-execution modes and both external routes", async () => {
  ...
  assert.match(manifest, /version: 4\.12\.0/);
  ...
  assert.match(manifest, /claude_with_codex:[\s\S]*?orchestrator: claude[\s\S]*?provider: codex-cli[\s\S]*?transport: exec[\s\S]*?model: gpt-6-sol[\s\S]*?reasoning_effort: high/);
...
test("project template requires an explicit valid role mode", async () => {
  ...
  assert.match(project, /strong: \{model: gpt-6-sol, reasoning_effort: high\}/);
  assert.match(project, /balanced: \{model: gpt-6-luna, reasoning_effort: high\}/);
  assert.match(project, /fast: \{model: gpt-5\.6-luna, reasoning_effort: medium\}/);
