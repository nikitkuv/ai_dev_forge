...
**Architecture:** Keep `.ai/` as the only committed framework payload and add `.ai/MIGRATE.md` as the executable migration entrypoint. A consumer stages the same payload as `.ai-next/`; Forge builds and validates candidate framework/router/adapter outputs, applies them with backup and rollback, and writes the first lock only after success. Node.js built-in contract tests enforce protected paths, local adapter ownership, overlay rendering, documentation consolidation, and absence of a duplicate migration payload.

**Tech Stack:** Markdown and YAML framework contracts, Node.js built-in `node:test`, Git sparse checkout, PowerShell and POSIX shell examples.
...
Create a `node:test` suite with a UTF-8 `read(path)` helper and these assertions:
...
```text
git add tests/migration-contract.test.mjs .ai/MIGRATE.md MIGRATION.md
...
Run: `node --test --test-name-pattern "router templates|adapter contract" tests/migration-contract.test.mjs`
...
```text
git add tests/migration-contract.test.mjs .ai/framework/manifest.yaml .ai/templates/adapters/codex/AGENTS.md .ai/templates/adapters/claude/CLAUDE.md .ai/05-create-platform-adapters.md .ai/framework/skills/forge-sync-adapters/SKILL.md
...
Run: `node --test --test-name-pattern "legacy migration|conformance allows" tests/migration-contract.test.mjs`
...
```text
git add tests/migration-contract.test.mjs .ai/framework/skills/forge-migrate-framework/SKILL.md .ai/framework/skills/forge-check-framework/SKILL.md .ai/06-final-validation.md .ai/BOOTSTRAP.md
...
Run: `node --test --test-name-pattern "general docs" tests/migration-contract.test.mjs`
...
```text
git add tests/migration-contract.test.mjs README.md RUNBOOK.md FRAMEWORK.md FRAMEWORK_DESIGN.md .ai/CONVENTIONS.md
