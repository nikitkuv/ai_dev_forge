...
1. резервирует `INT-NNNN` и пишет черновик `intents/INT-NNNN-shared-task-lists.md` первым durable-действием;
2. дозаполняет шаблон через короткое интервью и показывает собранную INT-запись на подтверждение;
3. уточняет target behavior;
4. после подтверждения создаёт `PLANNED/OUTLINE` Epic со ссылкой `Intent: INT-NNNN`;
5. показывает SPEC diff;
6. при необходимости показывает ARCHITECTURE/ADR diff;
7. не меняет active TASK;
8. переводит Epic в `READY` только после утверждения requirements и dependencies и отсутствия материальных открытых вопросов в INT.

Все будущие идеи сохраняются как Epic, а не как отдельный список «когда-нибудь». Отвергнутая идея остаётся INT-записью:

> **Пользователь:** Пожалуй, не надо — офлайн-режим важнее.

> **Оркестратор:** INT-0002 сохранён с `outcome: rejected` и rationale «отложено в пользу офлайн-режима». Epic не создан; при повторном похожем запросе я покажу эту запись.
