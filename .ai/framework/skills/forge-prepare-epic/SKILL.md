...
3. independently verify the proposal; never treat agent output as approval or canonical truth. Require each TASK's `Planned Change Map` under `task_change_map` in the framework contracts: inspected paths, actions and per-path reasons, indirect impact with verification coverage, and uncertainties with resolution steps. Check proposed new paths are labelled and unknown component or contract boundaries are resolved before Plan Approval. Link the maps from the ordered sequence and account for shared-component overlap in dependencies;
4. allocate project-global TASK IDs without restarting per Epic or reusing retired IDs; use one `records-write` packet with symbolic `{{id:alias}}` references for the complete approved plan/TASK batch. `next-id` is an inspection command, not an ID reservation. Keep drafts outside canonical workspaces until approval;
...
9. require `test_planning` coverage before Plan Approval: each TASK Verification Plan contains Planned Test Cases with stable IDs, AC/risk links, inputs, independently sourced expected outcomes, test level/target, reuse/extend/new classification and expected RED or justified non-RED rationale. The Epic Verification Plan contains Planned Epic Scenarios linking cross-Task behavior to expectations, owning Tasks and execution gates. Inspect existing coverage; block on unknown expectations or material coverage gaps, and assign missing harnesses/commands to resolution Tasks before dependent execution. Keep these lists proportional and within the existing artifacts; do not write executable tests during planning. Reserve the full project suite and unscoped global checks for Epic Validation;
...
Use `python .ai/tools/forge.py records-write <packet.json>` to render identifiers and preview all related files together; apply that exact token under the existing Plan Approval/Replan authorization. Use `source` for prepared draft files, and `expected_sha256` from `identity-check --path <existing-output>` (`file_hashes`) for existing outputs. Include reciprocal record changes in the same packet. Allocation changes invalidate the preview: regenerate the whole packet preview, never hand-renumber its parts. The helper validates the saved state and rolls back on failure. It does not grant approval or replace semantic planning. Run `validate --project` before handing the workspace to another session. Read the Record batches section of `.ai/tools/USAGE.md` for the packet schema.
