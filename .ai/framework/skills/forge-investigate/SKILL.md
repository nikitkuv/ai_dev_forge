...
1. Read relevant canonical documents, existing investigation files, current Epic/TASK state, repository instructions, code, tests, and Git status/diff. When the Python helpers are present, `python .ai/tools/forge.py query <terms> --kind INV,ADR` finds prior investigations and decisions on this subject — ranked pointers only; read the canonical records before reusing any conclusion. Preserve unrelated user work and report contradictions that affect the question.
...
3. Allocate the next monotonic `INV-NNNN` from the maximum ID under `investigations/`; do not fill gaps or reuse retired IDs. Create the record mechanically with `python .ai/tools/forge.py inv-create --subject <s> --area <a> [--paths ...]` (preview, then apply the reviewed token): it allocates the identifier, instantiates `.ai/templates/INVESTIGATION.md`, and records the baseline revision and dirty working-tree disposition before the first material experiment or change. Record the subject, area, relevant paths known so far, scope, exclusions, and authorization.
