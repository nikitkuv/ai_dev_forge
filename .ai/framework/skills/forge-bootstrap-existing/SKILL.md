...
3. Ask the `context-collector` for a local read-only inventory of code, tests, documentation, configuration, deployment material, issue artifacts, Git state, any existing product or architecture records, optional project-owned investigations and intents, and integration definitions without invoking their connectors.
...
- Do not create synthetic investigation or intent records, default hooks, MCP configuration, CLI dependencies, a local integration registry, or an external lifecycle layer. Preserve existing `investigations/`, `intents/`, and `.ai/integrations/` as project-owned and treat their absence as the clean baseline.
