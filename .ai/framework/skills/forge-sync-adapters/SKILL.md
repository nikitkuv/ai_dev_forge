...
4. Inventory unlisted agents, skills, `.codex/config.toml`, Claude settings, `opencode.json`, `.opencode/commands/`, `.opencode/plugins/`, `.opencode/skills/`, unlisted `.opencode/agents/`, hooks, project-owned investigations and intents, integration consumers, independent mutation history, and adjacent platform files as project-owned content.
...
Generate no `.ai/integrations/` or `quality/mutation-testing/` content. Generate no `investigations/` or `intents/` content. Embed no project-local INV or INT IDs, integration IDs, mutation records, provider names, scopes, bindings, or credentials. Generic investigation, profile/consumer, and mutation routing comes only from framework-owned sources.
...
4. verify additional project-owned agents, skills, `investigations/`, `intents/`, integration consumers, `quality/mutation-testing/` history, and all unlisted platform files are unchanged;
