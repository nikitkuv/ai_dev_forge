...
1. Run `python .ai/tools/forge.py context` and `validate --project`; consume every metadata page. For the active/paused TASK and Epic, obtain evidence-freshness verdicts from `evidence-check <TASK-path>` and `evidence-check --epic <EPIC-ID>` instead of recomputing fingerprints or comparing them by hand; treat `insufficient` and `stale` verdicts as evidence that must be rerun. Read `BACKLOG.md`, the selected active or paused plan, current TASK, relevant Git diff and linked ADR/INV sections. When a concrete question needs prior records beyond the explicit links, `python .ai/tools/forge.py query <terms>` returns ranked pointers across ADR/INT/INV/TASK and archived Backlog rows without loading bodies. Inventory every planned workspace through metadata; read bodies only for a concrete dependency, eligibility or consistency question. Optional integrations are read only for a relevant consumer. Do not invoke `context-collector` for successful mechanical recovery; use it only for unresolved cross-file interpretation. If Python is unavailable, collect the same bounded inventory with local tools without installing dependencies implicitly.
...
3. Determine:
   ...
   - unfinished or unresolved investigations, reciprocal `research_refs`, and any direct-fix paths that make existing TASK or Epic evidence stale;
...
- Rerun Epic Validation when aggregate evidence is missing, commands or profile gates are unresolved, the fingerprint changed, or a migrated legacy plan lacks the required verification contract.
...
- Treat INV records as canonical research evidence, never as lifecycle authority. Report broken reciprocal references and re-evaluate affected TASK/Epic evidence when `fixed_directly` paths changed after that evidence was recorded.
