...
Pass `--task <TASK-path>` to Python `checks` and external `role` calls so the helpers record measured duration, reused checks, role calls and available provider usage automatically. Do not estimate missing tokens or infer user acceptance from telemetry. Persist related canonical record edits through `records-write` (preview/apply) using existing authorization; status transitions still use the dedicated lifecycle commands. Before session handoff, run `validate --project` and resolve newly introduced mechanical inconsistencies at their source.
...
Assemble the Review Packet mechanically with `review-packet <TASK-path>` from the recorded path classification; the output carries recomputed whole/production fingerprints and scoped diffs. Verify evidence freshness and mechanical acceptance eligibility with `evidence-check <TASK-path>` — its verdicts never replace review-protocol, test-integrity, or acceptance judgment. Write approved check packets with `checks-new` instead of hand-authoring JSON.
...
For external planner/reviewer transport prefer `forge.py role` with the active orchestrator, role and `--assignment-file <path>`; the helper embeds the complete neutral role contract itself, so only the assignment passes through orchestrator context. It resolves the existing route and performs one bounded preflight internally. Do not execute a separate preflight and then repeat it inside the launcher. Legacy launcher paths below remain the compatibility transport if Python is unavailable. A started failure never changes provider or transport automatically.

Status transitions are mechanical edits: after each gate decision, execute it through `transition task <TASK-path> --to <status>` (preview first, then apply the reviewed token) instead of editing frontmatter or the Workflow State block by hand. The helper checks enums, transition legality and single-writer invariants; it never decides the gate.
...
3. Recheck the TASK's `Planned Change Map` against current code and completed dependencies, then show it with the TASK goal, scope, acceptance criteria, affected surface, risk flags, delivery track and rationale, Verification Plan, review focus, constraints, and any `external_sources` with their source intent. Record justified file-level refinements within approved boundaries; scope, component or contract expansion requires Replan. For a legacy TASK without a map, assess and record its paths and indirect impact before execution under the same rule; do not bulk-rewrite accepted history. Do not require connector access for an unlinked TASK.
...
Before execution, revalidate Planned Test Cases under `test_planning` against current requirements, code and completed dependencies. Fill missing cases for a legacy TASK within its existing approved boundaries before execution, without bulk-rewriting accepted history. Resolve unknown expected behavior and material coverage gaps; show relevant case refinements with the Task Start context. In-scope refinements need recorded reasons; scope or behavior-contract changes require Replan. A grant remains valid only under its existing unchanged-definition rules.

Pass the cases to the implementer and reconcile case IDs with actual test references, commands and observed results in Implementation Summary. Both standard tester verification and fast orchestrator assurance check this reconciliation, including dispositions for changed, removed or unexecuted cases. Planned expectations are not RED/GREEN evidence. Reuse existing regression tests without forcing artificial RED; implement new behavior one case at a time, not by writing the entire planned suite upfront.

1. Invoke `implementer` for this TASK only and pass the approved delivery track, its constraints, and the current Planned Change Map. Require production changes and necessary tests on both tracks. Reconcile returned actual paths and indirect impact with the map in Implementation Summary, recording unused planned paths, justified refinements and any Replan reference; a planned path alone never proves actual scope coverage.
